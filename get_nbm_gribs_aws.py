...
for init in date_range:
    ...
    try:
        ...
        obj_list = [f for f in obj_list if (int(f.split('.')[4].replace('f', ''))+1)%12 == 0]
        
    else:
        # NBM 4.0 PQPF on 0, 6, 12, 18
        obj_list = [f for f in obj_list 
                        if int(f.split('.')[4].replace('f', ''))%12 == 0]

    get_fhrs = [24, 48, 72, 96, 120, 144, 168]
    #get_fhrs = [120, 144, 168]    
...
#         (int(obj.split('.')[4].replace('f', '')) <= 180))]
        (int(obj.split('.')[4].replace('f', '')) in get_fhrs))]        
